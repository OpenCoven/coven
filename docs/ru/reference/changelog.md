...
- **Исправлено двойное нажатие клавиш в TUI на Windows.** `coven tui` и браузер сессий теперь реагируют только на события нажатия клавиш в Windows, что устраняет ошибку дублирования ввода, когда при наборе `a` появлялось `aa`, стрелки перемещались на две строки, а Enter срабатывал дважды. См. [Coven TUI](/start/coven-tui) и [Установка в Windows](/install/windows).
...
- **Больше никаких двойных нажатий в TUI на Windows.** Меню Coven TUI и браузер сессий теперь фильтруют только события `Press`. При наборе `a` больше не вставляется `aa`, стрелки переходят на одну строку за нажатие, а Enter активирует выбор один раз. На macOS и Linux поведение не меняется. См. [Coven TUI](/start/coven-tui).
