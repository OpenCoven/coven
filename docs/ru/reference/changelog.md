...
- **Больше никаких двойных нажатий в TUI на Windows.** Меню Coven TUI и браузер сессий теперь фильтруют только события `Press`, поэтому при наборе `a` больше не вставляется `aa`, стрелки переходят на одну строку за нажатие, а Enter активирует выбор один раз. На macOS и Linux поведение не меняется. См. [Coven TUI](/start/coven-tui).
