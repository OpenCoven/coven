...
- **Интеграции ввода / OpenClaw** — это поверхности приёма и оркестрации, которые могут передавать работу в тот же локальный runtime, не скрывая, что произошло.
...
- Передача приёма с поверхностей ввода в сессии comux/Coven.

### Путь интеграции OpenClaw / поверхностей ввода
...
- Держать копию о поверхностях ввода/OpenClaw честной: приём и оркестрация сидят над Coven; они не заменяют runtime-подложку.
...
- [ ] Приём с поверхностей ввода/OpenClaw создаёт или запрашивает задачу Coven
...
- Разделяй **runtime Coven**, **кокпит comux** и **приём с поверхностей ввода/OpenClaw**, чтобы люди понимали архитектуру.
