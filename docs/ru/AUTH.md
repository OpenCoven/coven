...
description: "Coven использует модель доступа через локальный Unix socket в рамках одного пользователя, а не OAuth или API-ключи. Узнай, что защищает /api/v1 и что требует удалённый доступ."
---
...
Это намеренно local-first MVP-поза. Она подходит для локальных клиентов того же пользователя, таких как CLI/TUI Coven, comux, OpenMeow и внешний OpenClaw-плагин. Это не схема auth для удалённого API.
