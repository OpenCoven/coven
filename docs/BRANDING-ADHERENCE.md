...
- [x] Purple reserved for CTAs, focus, and small state accents.
- [x] Hover uses border/surface-color changes instead of glow/scale.
...
3. **Public exports use the full logo; controlled landing chrome and reviewed banners have specific exceptions.** Public docs, README, packages, favicon/avatar, and OG exports use the full black-square, white-crown identity treatment; OG may composite the white crown on its black composition. Controlled dark landing navigation, hero, and footer may use the transparent `brand/logo/opencoven-mark.svg`. X/GitHub banner compositions are the reviewed social exception: the canonical crown silhouette appears in dim violet `--oc-purple-banner` (`#6A5FA0`) on black, while text, network art, and layout remain unchanged. This is not permission for arbitrary logo variants.
