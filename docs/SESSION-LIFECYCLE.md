...
- Adopted `POST /api/v1/adopted-sessions` commits the new row as `created` and its immutable launch-adoption record in the same transaction, before invoking the runtime. Immediately after cancellation ownership registration and before initial prompt delivery, the runtime invokes a terminal-safe callback exactly once to compare-and-set `created` to `running`. The exit writer may first move `created` or `running` to authoritative nonterminal `idle` for a successful conversation-grouped session, terminal `completed` for a successful ungrouped session, or terminal `failed` for a failed exit. A later activation CAS that finds any such winner returns false and does not overwrite it. A definitive runtime-establishment failure separately conditionally compare-and-sets only `created` to `failed`, preserving an `idle` or terminal winner; publication failure leaves the retained status ambiguous and never authorizes relaunch.
