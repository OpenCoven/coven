...
- **Brand-aligned TUI theme.** The Coven TUI now uses a unified theme aligned with the OpenCoven palette, with graceful fallbacks for no-color, 256-color, and truecolor terminals so it looks right whether you run it locally, over SSH, or inside CI. See [Coven TUI](/start/coven-tui).
