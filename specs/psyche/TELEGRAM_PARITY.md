...
**Companions:** [Product specification](./PRODUCT.md), [Technical architecture](./TECH.md), [Threat model](./THREAT_MODEL.md)

## Scope and clean-room rule

This ledger inventories the mature, user-observable Telegram behavior needed
for the Psyche goal. It was derived from public Telegram Bot API documentation,
...
No unlisted Bot API method is implied. A newly discovered reference behavior
...
| T-06 | Complete ingress at idempotent Coven turn adoption | Required | I,C | Client intent ID, request digest, and adoption lookup prevent duplicate sessions/turns after lost responses. |
...
| T-16 | Explicit non-idempotent `delivery_unknown` and audited resolution | Required | U,I,C,S,O | Accepted-but-response-lost is not claimed sent or blindly retried; recovery binds duplicate risk to a Coven decision and new effect. |
...
| T-20 | Per-effect Coven authorization decision | Required | U,I,S,C | Capability presence never authorizes dispatch, replies, pairing, approvals, or Telegram mutations. |
| T-21 | Typed effect digest and persisted decision binding | Required | U,I,S,C | Coven evaluates actual fields; restart/retry retains immutable effect, decision, policy/Ward revisions, and expiry. |
...
| R-18 | Per-topic familiar routing | Required | U,I,S,L | Each topic validates its own identity and Coven binding. |
...
| R-23 | OpenClaw ACP topic bindings and chat spawn commands | Rejected | U | Psyche uses Coven-native session/conversation bindings, not OpenClaw ACP concepts. |
...
| C-11 | Coven approval prompts and decisions | Required | U,I,S,L | Opaque, expiring, one-time callbacks; Coven decides. |
...
| D-11 | Delete message | Required | U,I,S,L | Destructive action requires Coven policy. |
...
| D-23 | Policy-controlled visible group replies | Required | U,I,S,L | Ambient or public replies require an explicit route/Coven policy. |
| D-24 | Pin requested delivery | Required | U,I,S,L | Requires chat permission and Coven action policy. |
...
| M-17 | Two-phase Coven-owned channel artifact ingestion | Required | U,I,S,C | Artifact admission precedes upload; opaque IDs then bind the exact turn without crossing a project path. |
| M-18 | Coven output-artifact streaming | Required | U,I,S,C,L | Outbound bytes stream by opaque session artifact after media-send authorization with full hash/size/type and identity bindings. |

## Telegram actions and service events

| ID | Capability | Class | Evidence | Decision |
|---|---|---|---|---|
| X-01 | Create forum topic | Required | U,I,S,L | Coven action policy required. |
| X-02 | Edit/close/reopen forum topic | Required | U,I,S,L | Destructive mutations are separately gated. |
| X-03 | Group-to-supergroup migration event | Required | U,I,L | Update route health and emit an operator proposal; do not rewrite config automatically. |
...
| O-04 | Polling, webhook, DNS, proxy, auth, and privacy diagnostics | Required | I,L,O | `psyche doctor` provides actionable redacted output. |
...
| O-09 | Dedicated-token canary | Required | L,O | Seven-day product gate before shared production cutover. |
...
| O-12 | Import numeric ACLs/routes through reviewed manifest | Required | U,S,O | Human-readable, secret-free, original schema. |
...
3. service objectives and release gates in `PRODUCT.md` pass;
4. threat-model security acceptance passes;
5. migration and rollback drills pass with one token owner; and
6. Val records explicit design approval before implementation and explicit
   parity approval before production cutover.
