#!/usr/bin/env python3
"""Generate Higgsfield batches with per-model params (no hardcoded flags).

Spec format:
{
  "name": "my-batch",
  "model": "kling3_0",                  # default model, scenes may override
  "defaults": {"aspect_ratio": "9:16", "duration": 5, "mode": "std"},
  "scenes": [
    {
      "id": "01",
      "slug": "agora-dawn",
      "prompt": "...",
      "image": "outputs/keyframes/agora.png",   # optional --image input
      "model": "veo3_1",                        # optional override
      "params": {"quality": "high"},            # merged over defaults
      "acceptance": ["..."]
    }
  ]
}

Only keys present in defaults/params are passed as flags, so each model
gets exactly the parameters it accepts.
"""
...
def command_for(spec: dict, scene: dict, verb: str) -> list[str]:
    model = scene.get("model") or spec.get("model")
    if not model:
        raise SystemExit("Spec needs a model (top-level or per scene).")
    params: dict = {**spec.get("defaults", {}), **scene.get("params", {})}
    cmd = [str(HF), "generate", verb, model, "--prompt", scene["prompt"]]
...
def dry_run(spec_path: Path, only: set[str] | None, limit: int | None) -> None:
    ...
    for scene in selected(spec["scenes"], only, limit):
        ...
        print(" ".join(command_for(spec, scene, "create")))
        for check in scene.get("acceptance", []):
            print(f"  - {check}")


def cost(spec_path: Path, only: set[str] | None, limit: int | None) -> None:
    spec = load_spec(spec_path)
    total = 0.0
    for scene in selected(spec["scenes"], only, limit):
        proc = run(command_for(spec, scene, "cost"))
        out = (proc.stdout + proc.stderr).strip()
        match = re.search(r"([\d.]+)\s*credits", out)
        if proc.returncode != 0 or not match:
            print(f"{scene_id(scene)}: ERROR {out}")
            continue
        credits = float(match.group(1))
        total += credits
        print(f"{scene_id(scene)} {scene.get('slug', '')}: {credits:g} credits")
...
def generate(spec_path: Path, only: set[str] | None, limit: int | None, no_wait: bool) -> None:
    ...
    run_dir = OUTPUTS / f"{spec.get('name', 'batch')}-{stamp}"
    ...
    for scene in selected(spec["scenes"], only, limit):
        ...
        proc = run(command_for(spec, scene, "create"))
        ...
        if proc.returncode != 0:
            ...
            manifest["scenes"].append({"id": key, "slug": scene.get("slug"), "error": True})
            continue
        ...
        if not no_wait:
            try:
                ...
                manifest["scenes"].append(
                    {"id": key, "slug": scene.get("slug"), "job_id": job_id, "error": True}
                )
                continue
...
def main() -> int:
    ...
    if args.cmd == "dry-run":
        ...
        cost(args.spec, only, args.limit)
    else:
