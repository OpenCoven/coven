...
export function createCovenPluginConfigSchema() {
  return {
    safeParse(value: unknown) {
      const result = CovenPluginConfigSchema.safeParse(value);
      if (result.success) {
    ...
    parse(value: unknown) {
      return CovenPluginConfigSchema.parse(value);
    },
    jsonSchema: {
      type: "object",
      ...
      properties: {
        covenHome: {
          type: "string",
          description:
            "Path to the Coven daemon state directory. Defaults to ~/.coven; environment variables are not used for this trust anchor.",
        },
        socketPath: {
          type: "string",
          description:
            "Path to the Coven daemon Unix socket. Defaults to <covenHome>/coven.sock; overrides must resolve to that fixed socket filename.",
        },
        allowFallback: {
          type: "boolean",
          description:
            "When true, fall back to fallbackBackend if Coven is unavailable or launch fails. Defaults to false.",
        },
        fallbackBackend: {
          type: "string",
        ...
        harnesses: {
          ...
          description:
            "Explicitly map additional OpenClaw ACP agent ids to authorized Coven harness ids. Unknown agent ids are rejected.",
        },
      },
    },
  };
